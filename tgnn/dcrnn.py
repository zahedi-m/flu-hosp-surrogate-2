...
class DConv(MessagePassing):
    ...
    def calculate_norms(self, edge_index, edge_weight):
        eps= 1e-8
        row, col = edge_index
        # Example normalization calculation, adjust based on the specific normalization technique
        out_deg = torch_scatter.scatter_add(edge_weight, row, dim_size=edge_index.shape[-1])
        in_deg  = torch_scatter.scatter_add(edge_weight, col, dim_size=edge_index.shape[-1])
    ...
    def forward(
        ...
        reverse_edge_index= edge_index.t()
